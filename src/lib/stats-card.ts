...
// Constants
export const CARD_MIN_WIDTH = 287;
export const CARD_DEFAULT_WIDTH = 287;
export const RANK_CARD_MIN_WIDTH = 420;
export const RANK_CARD_DEFAULT_WIDTH = 450;
export const RANK_ONLY_CARD_MIN_WIDTH = 290;
export const RANK_ONLY_CARD_DEFAULT_WIDTH = 290;

export const LONG_LOCALES = [
  "az",
  "bg",
  "cs",
  "de",
...
export const kFormatter = (num: number, precision?: number): string => {
  if (num >= 1000000) {
    return (num / 1000000).toFixed(precision ?? 1) + "M";
  }
  if (num >= 1000) {
    return (num / 1000).toFixed(precision ?? 1) + "k";
  }
  return num.toString();
};

export const measureText = (str: string, fontSize: number = 10): number => {
  const widths = [
...
export const calculateCircleProgress = (
  value: number,
  radius: number = 40,
): number => {
  const c = Math.PI * (radius * 2);
  const clampedValue = clampValue(value, 0, 100);
  return ((100 - clampedValue) / 100) * c;
};
...
export const icons = {
  ...
  fire: `<path fill-rule="evenodd" d="M5.016 16.945c-.966 0-1.75-.784-1.75-1.75v-2.5a.75.75 0 0 1 1.5 0v2.5c0 .138.112.25.25.25h5.5a.25.25 0 0 0 .25-.25v-2.5a.75.75 0 0 1 1.5 0v2.5c0 .966-.784 1.75-1.75 1.75h-5.5Zm-1.5-5.5v-2.5a.75.75 0 0 1 1.5 0v2.5c0 .138.112.25.25.25h5.5a.25.25 0 0 0 .25-.25v-2.5a.75.75 0 0 1 1.5 0v2.5c0 .966-.784 1.75-1.75 1.75h-5.5c-.966 0-1.75-.784-1.75-1.75ZM7.75 1.5a.75.75 0 0 1 .75.75v2.5h2.5a.75.75 0 0 1 0 1.5h-2.5v2.5a.75.75 0 0 1-1.5 0v-2.5h-2.5a.75.75 0 0 1 0-1.5h2.5v-2.5a.75.75 0 0 1 .75-.75Z" />`,
};
